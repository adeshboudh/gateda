...
**Where this sits:** Calculus & Optimization $\rightarrow$ *Maxima, Minima & Optimization*. Module **3 of 3** in Subject 7 — **the final module of the entire GATE DA syllabus.**
...
### A. Critical points
An interior **critical point** is where $f'(x)=0$ (or $f'$ is undefined). These are the only interior candidates for local extrema. *(Necessary, not sufficient: $f'(x^*)=0$ can be an inflection, e.g. $x^3$ at $0$.)*

### B. First-derivative test
At a critical point $x^*$: if $f'$ changes $+\to-$, it's a **local max**; $-\to+$, a **local min**; no sign change $\Rightarrow$ neither.

### C. Second-derivative test
If $f'(x^*)=0$:
$$f''(x^*)>0\Rightarrow\text{local minimum},\qquad f''(x^*)<0\Rightarrow\text{local maximum},\qquad f''(x^*)=0\Rightarrow\text{inconclusive}.$$
*(2024 Q15.)*
...
For continuous $f$ on $[a,b]$ (Extreme Value Theorem guarantees a max and min exist): evaluate $f$ at **all interior critical points AND both endpoints**, then pick the largest and smallest values. On an **open / half-open** interval, an excluded endpoint may not attain an extremum. *(2025 Q49, 2026 Q27.)*
...
- **But a minimum need not exist** — e.g. $f(x)=e^x$ has $f''>0$ yet no minimum. *(2025 Q51.)*
...
1. $f''(x^*)>0$ gives a **local** min — not necessarily global. *(2024 Q15.)*
2. On a closed interval, **always check endpoints**, not just critical points. *(2025 Q49.)*
3. **Convex ($f''>0$) does not guarantee a minimum exists.** *(2025 Q51.)*
4. $f''(x^*)=0\Rightarrow$ second-derivative test **inconclusive** (use the first-derivative test).
5. $f'(x^*)=0$ is necessary but **not sufficient** ($x^3$ at $0$ is an inflection).
6. “Maximum value” means **global** on the given domain — watch endpoint ties. *(2026 Q27.)*
...
Evaluate $f$ at every interior critical point **and** at $a,b$; the largest value is the global max, the smallest the global min. *(2025 Q49.)*

### Convexity questions
$f''>0$ everywhere $\Rightarrow$ strictly convex $\Rightarrow f'$ strictly increasing $\Rightarrow$ at most one critical point (a min **if** it exists); a local min is global — but **existence is not guaranteed**. *(2025 Q51.)*
...
### Example 1 — The second-derivative test *(2024 Q15 · MCQ)*
**Q.** For a twice-differentiable $f:\mathbb{R}\to\mathbb{R}$, if at some $x^*$ we have $f'(x^*)=0$ and $f''(x^*)>0$, then $f$ necessarily has a ____ at $x^*$. (A) local minimum (B) global minimum (C) local maximum (D) global maximum.

**Solve.** $f'(x^*)=0$ makes $x^*$ a critical point; $f''(x^*)>0$ means the curve is concave up there, so $x^*$ is a **local minimum**. The derivative values at a single point give only **local** information — nothing forces it to be global.

**Answer: (A) local minimum.**

---

### Example 2 — Classify the critical points *(2024 Q50 · MSQ)*
...
### Example 3 — Global extrema on a closed interval *(2025 Q49 · MSQ)*
...
**Answer: (C) and (D).** *Lesson:* the extrema are at the **endpoints**, not the interior critical points.

---

### Example 4 — Convex function properties *(2025 Q51 · MSQ)*
...
**Q1. ★ (MCQ)** At a critical point, if $f''(x^*)<0$, then $x^*$ is a
...
**Q3. ★★ (NAT)** The $x$-value at which $f(x)=x^2-6x+5$ attains its minimum is __________ .
...
**Q7. ★★ (NAT)** The number of critical points of $f(x)=2x^3-3x^2-12x+5$ is __________ .
...
**Q9. ★★ (NAT)** The maximum value of $f(x)=-x^2+4x+1$ is __________ .
...
- **≤5:** re-read Part 1 C–F; lock in *$f''>0\Rightarrow$ local min*, *check endpoints for global extrema*, and *convex $\ne$ has-a-minimum*.
