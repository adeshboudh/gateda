...
**Where this sits:** Linear Algebra $\rightarrow$ *Singular Value Decomposition & LU Decomposition* — the matrix-factorization finale. Module **6 of 6** in Subject 5 — **this completes Linear Algebra.**
...
> **Why it matters:** one bridge fact cracks most SVD questions — **singular values are $\sqrt{\text{eigenvalues of }A^\top A}\ge 0$**, and for a **symmetric PSD** matrix they simply *equal* the eigenvalues. That single idea connects this module to eigenvalues (5.3), projections (5.4), and quadratic forms (5.5).
...
- Symmetric case $\mathbf u\mathbf u^\top$ (PSD): single nonzero eigenvalue $=$ singular value $=\mathbf u^\top\mathbf u=\lVert\mathbf u\rVert^2$ (eigenvector $\mathbf u$). *(2024 Q61.)*
...
### F. LU Decomposition *(efficient coverage — no PYQ 2024–26)*
...
### Example 1 — Singular values of a rank-1 matrix *(2024 Q61 · NAT)*
...
**Answer: $55$.** *Trap avoided:* only **one** singular value is nonzero — a rank-1 matrix.

---

### Example 2 — Singular values of a projection *(2025 Q50 · MSQ · cross-ref Module 5.4)*
...
### Example 4 — LU decomposition & determinant *(original · Med)*
...
**Answer:** $A=LU$ as above, $\det A=6$. *Method:* Gaussian-elimination multiplier $\to L$, reduced matrix $\to U$, $\det=\prod U_{ii}$.
...
**Q4. ★★ (NAT)** For $\mathbf u=(2,1,2)^\top$, the only nonzero singular value of $\mathbf u\mathbf u^\top$ is __________ .
...
**Q6. ★★ (NAT)** For $A=\begin{bmatrix}5&0\\ 0&-12\end{bmatrix}$, the largest singular value is __________ .
...
**Q9. ★★★ (NAT)** Using LU (no pivoting), the determinant of $A=\begin{bmatrix}1&2\\ 3&4\end{bmatrix}$ is __________ .
...
- **≤5:** re-read Part 1 B–C; lock in *$\sigma=\sqrt{\lambda(A^\top A)}\ge0$*, *symmetric PSD $\Rightarrow\sigma=\lambda$*, and *rank-1 has one nonzero $\sigma=\lVert\mathbf u\rVert\lVert\mathbf v\rVert$*.
