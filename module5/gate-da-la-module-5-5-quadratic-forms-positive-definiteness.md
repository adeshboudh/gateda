...
**Where this sits:** Linear Algebra $\rightarrow$ *Quadratic Forms & Positive-Definiteness* — the $\mathbf x^\top A\mathbf x$ machinery. Module **5 of 6** in Subject 5.
...
- PD $\Rightarrow$ invertible, $\det>0$, all diagonal entries $>0$ (but $\det>0$ **alone** is *not* sufficient — two negative eigenvalues also give $\det>0$).
...
- **PD $\iff$ columns of $X$ are linearly independent** (then $X\mathbf z=\mathbf 0$ only for $\mathbf z=\mathbf 0$). So a Gram matrix of independent vectors is PD $\Rightarrow$ invertible, $\det>0$, no zero singular value. *(2025 Q38.)*
...
- On the **unit sphere** ($\mathbf x^\top\mathbf x=1$): $\displaystyle\max_{\lVert\mathbf x\rVert=1}\mathbf x^\top A\mathbf x=\lambda_{\max}$ (attained at the top eigenvector) and $\displaystyle\min=\lambda_{\min}$. *(2026 Q65.)*
...
2. **PD $\ne$ “$\det>0$”** alone — check *all* eigenvalues / *all* leading minors.
...
### Example 1 — Gram matrix of independent vectors *(2025 Q38 · MCQ)*
...
### Example 2 — Rayleigh max on the unit sphere *(2026 Q65 · NAT)*
...
**Answer: $1$.** *Method:* recognize the structured matrix $\to$ its top eigenvalue.

---

### Example 3 — Classify definiteness *(original · Med)*
...
**Answer: yes, positive definite.** *Method:* Sylvester's leading-minor test (confirmed by eigenvalues).

---

### Example 4 — Range of a quadratic form *(original · Med)*
...
**Answer: max $=4$, min $=2$.** *Method:* Rayleigh quotient is bounded by the extreme eigenvalues.
...
**Q3. ★★ (NAT)** For $A=\begin{bmatrix}4&0\\ 0&9\end{bmatrix}$, the maximum of $\mathbf x^\top A\mathbf x$ over $\mathbf x^\top\mathbf x=1$ is __________ .

**Q4. ★★ (NAT)** For the same $A$, the minimum of $\mathbf x^\top A\mathbf x$ over the unit sphere is __________ .
...
**Q8. ★★★ (NAT)** For $A=\begin{bmatrix}2&1\\ 1&2\end{bmatrix}$, the minimum of $\mathbf x^\top A\mathbf x$ subject to $\mathbf x^\top\mathbf x=1$ is __________ .
...
- **≤5:** re-read Part 1 B–E; lock in *definiteness $=$ eigenvalue signs*, *Gram $=$ PSD (PD iff independent)*, and *unit-sphere max/min $=\lambda_{\max}/\lambda_{\min}$*.
