...
**Where this sits:** Linear Algebra $\rightarrow$ _Vector Spaces, Subspaces, Linear Independence & Basis_ — the foundation of the whole subject and the **first module of Subject 5**.
...
> **Why it matters:** these are _free marks_ once the **subspace test** and the independence/basis facts are automatic — and they are prerequisites for rank–nullity (5.2), eigenvectors (5.3), and projections (5.4).
...
A span is **always** a subspace. **Trap:** if the coefficients are restricted — squared ($\alpha^2$), absolute value, or sign-constrained ($\alpha \ge 0$) — the set is generally **not** a subspace (closure under _negative_ scalars fails).
...
- **Bases are not unique** — _any_ $n$ linearly independent vectors in $\mathbb{R}^n$ form a basis. (Dimension **is** unique.)
...
1. **Zero test first.** Does $\mathbf 0$ satisfy the defining condition? If **no** $\to$ _not a subspace_, stop. (Instantly catches every affine set.)
...
- **Dimension from constraints:** in $\mathbb{R}^n$, each _independent_ homogeneous linear equation cuts dimension by 1 — so a single equation gives a hyperplane of dimension $n-1$.
...
**Answer: (A) and (C).** _Method:_ zero test + linear-vs-restricted coefficients.

---

### Example 2 — Orthonormal bases & independence _(2025 Q25 · MSQ)_
...
**Answer: (B) and (D).** _Method:_ orthonormal $\Rightarrow$ independent (one-way); bases are not unique.

---

### Example 3 — Subspace $\cap$ ball _(2026 Q22 · MCQ)_
...
**Answer: (A) $16\pi$.** _Trap avoided:_ $\mathbf x^\top\mathbf x\le16$ means radius $4$, not $16$.
...
**Q4. ★★ (NAT)** The dimension of $\operatorname{span}\{(1,2),\,(2,4)\}$ in $\mathbb{R}^2$ is \***\*\_\_\*\*** .
...
**Q7. ★★ (NAT)** The number of vectors in any basis of $\{\mathbf x\in\mathbb{R}^4 : x_1+x_2+x_3+x_4=0\}$ is \***\*\_\_\*\*** .
...
- **≤5:** re-read Part 1 A–D; lock in _affine $\ne$ subspace_, _independent $\ne$ orthonormal_, and _dimension $=$ pivots_.
