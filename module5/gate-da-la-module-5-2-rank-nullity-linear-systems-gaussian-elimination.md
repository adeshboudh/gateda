...
**Where this sits:** Linear Algebra $\rightarrow$ _Rank, Nullity & Systems of Linear Equations (Gaussian elimination)_ — the engine that decides **when a linear system has no / one / infinitely many solutions**. Module **2 of 6** in Subject 5.
...
> **Why it matters:** one identity — **rank $+$ nullity $=$ number of columns** — plus the **augmented-rank criterion** answers almost every system question. Master the _shape intuition_ (square / wide / tall) and the MSQ existence questions become instant.
...
- The solution set _is_ the null space, of dimension $n-r$.

### F. Gaussian elimination & its cost

**Algorithm:** _forward elimination_ (use each pivot to zero entries below it) $\to$ row echelon (upper-triangular) form, then _back-substitution_.
...
Back-substitution alone costs $\sum_{i=1}^{n}(n-i)=\dfrac{n(n-1)}{2}=O(n^2)$. _(This is exactly 2025 Q12.)_

### G. Shape intuition (kills the MSQ existence questions)

- **Square $n\times n$:** either invertible (every $\mathbf b\to$ unique) **or** singular (some $\mathbf b\to$ none, others $\to$ infinite). You can **never** get "unique for one $\mathbf b$, infinite for another" — uniqueness for _any_ RHS forces invertibility, making _all_ RHS unique.
...
3. $\det A = 0 \Rightarrow$ **no** solution _or_ **infinitely many** (never unique) — which one depends on $\mathbf b$.
...
### Example 1 — Cost of Gaussian elimination _(2025 Q12 · MCQ)_
...
**Answer: $O(n^2)$ — option (B).** _Contrast:_ a **general** dense $n\times n$ system needs forward elimination too, costing $O(n^3)$.

---

### Example 2 — Row sums equal 1 _(2025 Q13 · MCQ)_
...
**Answer: (C) infinitely many solutions.** _Method:_ find one nonzero null vector $\Rightarrow$ singular $\Rightarrow$ infinite.

---

### Example 3 — Which solution patterns can coexist? _(2024 Q48 · MSQ)_
...
- **(A)** Square $3\times3$: unique for some RHS $\Rightarrow M$ invertible $\Rightarrow$ _every_ RHS is unique — can't also be infinite. **False.**
...
### Example 4 — Parameter for infinitely many _(original · Med)_
...
**Answer: $a=4$.** _Method:_ drive the parameter into the last pivot; set it to $0$ for the degenerate case (and confirm consistency).
...
**Q2. ★ (NAT)** If $A\in\mathbb{R}^{5\times5}$ has rank $3$, its nullity is \***\*\_\_\*\*** .
...
**Q5. ★★ (NAT)** The system $x+y=2,\ 2x+2y=5$ has \***\*\_\_\*\*** solution(s).
...
**Q8. ★★ (NAT)** The dimension of the solution space of $\{\,x+2y+3z=0,\ 2x+4y+6z=0,\ x+y+z=0\,\}$ is \***\*\_\_\*\*** .
...
**Q7 — (C).** $\det=0\Rightarrow A$ singular $\Rightarrow$ no solution _or_ infinitely many, depending on $\mathbf b$; never unique.
...
**Q10 — (D) $O(n^3)$.** Forward elimination dominates at $O(n^3)$; back-substitution adds only $O(n^2)$. (Compare 2025 Q12: an _already_ upper-triangular matrix skips forward elimination and costs only $O(n^2)$.)
...
- **≤5:** re-read Part 1 C–E; lock in _rank $+$ nullity $=$ columns_, _consistency needs equal augmented rank_, and _wide systems are never unique_.
