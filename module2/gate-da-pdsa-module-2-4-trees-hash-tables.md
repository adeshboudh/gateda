...
**Where this sits:** Programming, Data Structures & Algorithms $\rightarrow$ _Trees & Hash Tables_ — the non-linear and associative structures. **Tree traversals are a flagged recurring pattern.**
...
- **2024 Q28** — which **traversal pairs** uniquely rebuild a _full_ binary tree
...
Root, parent/child, **leaf** (no children), **internal** node (has children), **subtree**. **Depth** of a node = edges from the root; **height** of the tree = edges on the longest root-to-leaf path (so a single node has height $0$). _(Some texts count nodes instead — watch which convention a question uses.)_
...
**Reconstruction:** _preorder + inorder_ or _postorder + inorder_ uniquely rebuild **any** binary tree. _preorder + postorder_ works **only for full** binary trees (otherwise a one-child node is ambiguous). **Inorder alone is never enough.**
...
### Example 1 — BST construction and its three traversals _(original · Easy–Med)_
...
_Method:_ recurse the order definitions; the inorder doubles as a sortedness check.

---

### Example 2 — Reconstruct a tree, then give postorder _(2026 Q25 · MSQ · Med)_
...
**Answer: (A) and (B).** _Method:_ root from preorder, split via inorder, recurse, then read postorder.

---

### Example 3 — Hashing with linear probing _(2025 Q18 · MCQ · Med)_
...
**Answer: (D) 4 and 6.** _Method:_ on a collision, linear probing tries the next slots $h+1, h+2, \dots \pmod{m}$.

---

### Example 4 — Min-heap insert and extract-min _(original · Med)_
...
**Result:** extracted `1`; heap becomes `[3, 5, 8]`. _Method:_ insert = append + sift up; extract = root out, last to root, sift down — each $O(\log n)$.
...
**Q3. ★★ (MSQ)** Which traversal pairs are sufficient to uniquely reconstruct a **full** binary tree? _(2024 Q28)_
(A) preorder + inorder (B) inorder + postorder (C) preorder + postorder (D) inorder only

**Q4. ★★ (NAT)** A full binary tree has 5 internal nodes. The number of leaf nodes is ****\_\_**** .

**Q5. ★★ (NAT)** The keys `5, 3, 8, 1, 2` are inserted one by one into an initially empty **min-heap**. The key at the root is ****\_\_**** .

**Q6. ★★ (NAT)** A hash table of size 7 uses $h(x) = x \bmod 7$ with **separate chaining**. After inserting `10, 17, 24`, the number of keys in the chain at bucket 3 is ****\_\_**** .
...
**Q10. ★★ (NAT)** The maximum number of nodes in a binary tree of height $3$ (height = edges on the longest path) is ****\_\_**** .
