...
**Where this sits:** Probability & Statistics → _Continuous Distributions_ — uniform, exponential, and (most importantly) the **normal/standard-normal**. The normal underpins the CLT (1.7), confidence intervals and hypothesis testing (1.8), and the normal approximations in 1.4.
...
- **Parameterization warning:** the _rate_ is $\lambda$ and the _mean_ is $1/\lambda$. Some GATE problems state the **mean** directly (e.g. 2026 Q34 says "mean $\lambda$") — read which one is given.
...
- **Exponential:** use the tail $P(X > x) = e^{-\lambda x}$; for "given it has lasted $s$", use **memorylessness**; to _find_ $\lambda$, set the tail/CDF equal to the given probability and solve (2025 Q21).
...
### Example 1 — Uniform: probability, mean, variance _(original · Easy–Med)_
...
_Method:_ uniform probability is a length ratio; mean/var from the formulas.

---

### Example 2 — Exponential rate from a tail probability _(2025 Q21 · MCQ · Med)_
...
**Answer: (A) $\ln 2$.** _Trap:_ stopping at $2\lambda = \ln 4$ and picking (B); you must halve it. _(Memoryless bonus: the same exponential gives $P(X>5 \mid X>2) = P(X>3)$ directly — see Module 1.2 Q3.)_

---

### Example 3 — Normal: standardize and use Φ _(original · Med)_
...
**Answer: ≈ 0.7745.** _Method:_ take $\sqrt{\ }$ of the variance → standardize → $\Phi$ difference, using $\Phi(-z)=1-\Phi(z)$.

---

### Example 4 — Standard normal linear transform _(2025 Q20 · MCQ · Med)_
...
**Answer: (A) $a = -2, b = 1$.** _Method:_ match moments one at a time; the cross-moment $E[(X-\mu)Z] = a$ pins down the sign that variance alone can't.
...
**Q2. ★ (NAT)** If $X \sim \text{Uniform}(2, 8)$, then $P(X > 5) =$ ****\_\_**** .

**Q3. ★★ (MCQ)** A continuous $X$ has CDF $F(x) = \dfrac{x-t}{4-t}$ for $t \le x \le 4$ (0 below, 1 above). If the median of $X$ is 3, then $t =$ _(2025 Q19)_
(A) 2 (B) 1 (C) −1 (D) 0

**Q4. ★★ (NAT)** If $X \sim$ Exponential with $\lambda = 0.5$, then $P(X < 2) =$ ****\_\_**** (two decimals).

**Q5. ★★ (MCQ)** $X$ is exponential with $P(X > 3) = 0.5$. Then $P(X > 5 \mid X > 2) =$
(A) 0.25 (B) 0.5 (C) 0.75 (D) 1.0

**Q6. ★★ (NAT)** If $X \sim \text{Normal}(\mu = 50, \sigma^2 = 25)$, then $P(X > 60) =$ ****\_\_**** (two decimals).

**Q7. ★★ (NAT)** $X \sim \text{Uniform}[1, 3]$ and $Y \sim \text{Uniform}[2, 4]$ are independent. Then $P(X \ge Y) =$ ****\_\_**** (three decimals). _(2024 Q56)_
...
**Q9. ★★ (NAT)** If $X \sim \text{Normal}(2, 9)$, then $\mathrm{Var}(3X - 1) =$ ****\_\_**** .

**Q10. ★★★ (NAT)** $X \sim \text{Normal}(0, 9)$ and $Y \sim \text{Normal}(0, 16)$ are independent. Then $P(X + Y > 5) =$ ****\_\_**** (two decimals).
