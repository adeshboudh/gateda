...
**Where this sits:** Artificial Intelligence $\rightarrow$ *First-Order / Predicate Logic*. Module **4 of 5** in Subject 6.
...
**The #1 trap:** writing $\forall x\,(P(x)\wedge Q(x))$ for “all P are Q” — that wrongly says *everything* is both $P$ and $Q$. Likewise $\exists x\,(P(x)\to Q(x))$ is almost always wrong (it's vacuously satisfied by any non-$P$ object).
...
- **$\forall x\,\exists y \not\equiv \exists y\,\forall x$** — order matters for mixed quantifiers.
  - $\forall x\,\exists y\,\text{Loves}(x,y)$: everyone loves *someone* ($y$ may depend on $x$).
...
$\forall x\,(P(x)\to Q(x))$. If you see $\forall$ paired with $\wedge$, suspect an error. *(2026 Q14.)*
...
Split into (non-$R\to$ property) $\wedge$ ($R\to\neg$ property); rewrite each conjunct with its contrapositive to match the given options (so two different-looking forms can both be correct). *(2024 Q54.)*
...
### Example 1 — Spot the wrong translation *(2026 Q14 · MCQ)*
...
- (C) **Wrong.** “Each king is a person” must be $\forall x\,(\text{IsKing}(x)\Rightarrow\text{IsPerson}(x))$. The $\wedge$ form claims *everything* is both a king and a person.
- (D) correct — the standard $\forall x\,(P\Rightarrow Q)$ pattern.

**Answer: (C).** *Trap:* $\forall$ must pair with $\to$, not $\wedge$.

---

### Example 2 — Quantifier validity *(2026 Q48 · MSQ)*
...
### Example 3 — “All P except R” *(2024 Q54 · MSQ)*
...
**Answer: (A) and (C).** *Key insight:* $s_1\equiv s_2$ and $s_3\equiv s_4$ by contrapositive, so (A) and (C) are the same statement.

---

### Example 4 — Negate a quantified statement *(original · Med)*
...
**Answer:** $\exists x\,(\text{Student}(x)\wedge\neg\text{Passed}(x))$ — “there is a student who did not pass.” *Method:* $\neg\forall=\exists\neg$, and $\neg(P\to Q)=P\wedge\neg Q$.
