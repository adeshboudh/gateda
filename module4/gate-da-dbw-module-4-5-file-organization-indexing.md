...
**Where this sits:** Database & Warehousing $\rightarrow$ *File Organization & Indexing* — how data is stored on disk and searched quickly. **B+ tree indexing is a flagged recurring pattern.**
...
$$n \cdot p + (n-1) \cdot k \le B.$$
  ...
  *(Non-leaf nodes hold only **node pointers** and keys — no data-record pointers.)*
...
### Example 1 — B+ tree non-leaf node capacity *(2026 Q32 · NAT · Med)*
...
**Answer: 195.** *Trap:* the data-record-pointer size (12) is a distractor — those live only in leaves.

---

### Example 2 — Choosing the right index *(2024 Q55 · MCQ · Med)*
...
**Answer: (B).** *Method:* equality $\to$ hash, range $\to$ B+ tree.

---

### Example 3 — A B+ tree leaf split *(original · Med)*
...
```
**Result:** root `[30]`; leaves `[10,20]` and `[30]`. *Method:* overflow $\to$ split $\to$ copy a separator up (creating a root here).

---

### Example 4 — Insertion with a cascading split *(2026 Q41 · MCQ · Hard)*
...
**Answer: (A) 5.** *(Note: the exact key that ends up promoted depends on the B+ tree split convention; the net effect here is a height increase with a single-key root. Trace per your textbook's convention.)* *Method:* leaf overflow $\to$ split $\to$ cascade $\to$ root split $\to$ new single-key root.
...
**Q3. ★★ (NAT)** A B+ tree non-leaf node block is 1024 bytes; node-pointer size 8, key size 8. The maximum number of node pointers in a non-leaf node is __________ .
...
**Q8. ★★ (NAT)** A B+ tree non-leaf block is 4096 bytes; node-pointer size 8, key size 16. The maximum number of node pointers is __________ .
