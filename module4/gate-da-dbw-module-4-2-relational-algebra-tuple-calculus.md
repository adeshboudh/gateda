...
**Where this sits:** Database & Warehousing $\rightarrow$ _Relational Algebra & Tuple Calculus_ — the formal query languages underlying SQL.
...
2. **Division is the "for all" operator** — spot "who is related to _all_..." wording.
...
### Example 1 — Basic select / project / join _(original · Easy–Med)_
...
_Method:_ $\sigma$ filters rows, $\pi$ keeps columns (deduplicated), $\bowtie$ matches the common attribute `sid`.

---

### Example 2 — Subset check by set difference _(2024 Q26 · MCQ · Med)_
...
**Answer: (C).** _Method:_ "every $A$ in $B$" $\iff A \setminus B = \varnothing$, with $B$ the **union**.

---

### Example 3 — The division operator _(2025 Q62 · NAT · Hard)_
...
**Answer: 1** (only Karteek). _Method:_ division $=$ "paired with every divisor value."

---

### Example 4 — Tuple relational calculus _(2026 Q59 · NAT · Med)_
...
**Answer: 2.** _Method:_ drop the trivially-true quantifier; filter $X$ by the meaningful $\exists m \in Y$ predicate.
...
**Q3. ★★ (MCQ)** The expression $\pi_{owner}(\text{Own} \bowtie \sigma_{color='red'}(\text{Car} \bowtie \sigma_{maker='ABC'}(\text{Make})))$ computes _(2025 Q17)_
...
**Q4. ★★ (NAT)** $R(A,B) = \{(1,2),(3,4)\}$, $S(B,C) = \{(2,5),(2,6),(4,7)\}$. The number of tuples in $R \bowtie S$ is \***\*\_\_\*\*** .
...
**Q6. ★★★ (MCQ)** Evaluate $\Pi_{P,R,S}\big[\sigma_{Q=Q3 \vee R=R2}(X \bowtie Y) \bowtie \sigma_{S>1}(Y \bowtie Z)\big]$ on the 2026 instances of $X,Y,Z$. The result has _(2026 Q42)_
...
**Q8. ★★ (NAT)** A relation has a `dept` column with values CS, CS, EE, ME, CS. The number of tuples in $\pi_{dept}$ of this relation is \***\*\_\_\*\*** .
...
**Q9 — (B) declarative.** TRC specifies _what_ tuples to retrieve via a predicate, not _how_.
